...
Game.prototype.init = function() {
	...
	this.start_message = "Натисни пробіл для старту"
...
Game.prototype.update_hud = function() {
	...
	context.fillText("Очки", 100, 80);
	context.fillText(score_string, 170, 80);

	context.fillText("Час", 100, 110);
	context.fillText(this.get_simple_time(), 170, 110);

	context.fillText("Пальне", 100, 140);
	...
	context.fillText("Висота", window.innerWidth - 120, 80);
	context.fillText(a_string, window.innerWidth - 50, 80);

	context.fillText("Горизонтальна швидкість", window.innerWidth - 120, 110);
	context.fillText(h_string, window.innerWidth - 50, 110);

	context.fillText("Вертикальна швидкість", window.innerWidth - 120, 140);
...
Game.prototype.handle_landing = function() {
	...
	if (this.landed) {
		...
		if (current_time - this.landing_time > 2000 && !this.assigned_score) {
			...
			else if (this.landing_vel > 16 && this.landing_vel < 32) {
				this.message = "Трохи зажорстко, але уже краще!!!";
				this.game_status = "GAME OVER";
				this.score += 50;
				this.landing_type = 1;
